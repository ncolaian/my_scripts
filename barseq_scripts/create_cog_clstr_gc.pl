...
my ($genes_to_cogs_href, $ann_to_cogs_href) = create_gene_2_cog_link(\@annote_file_array);
...
sub create_gene_2_cog_link {
    ...
    foreach my $line_aref ( @$annote_aref ) {
        $hash{$line_aref->[$locus_col]} = $line_aref->[$clstr_col];
        $clst_cog{$line_aref->[$clstr_col]} = $line_aref->[$cog_col];
    }
    
    return (\%hash, \%clst_cog);
}

sub create_clstr_2_cogs_link {
    my ( $cog_file ) = @_;
    ...
    foreach my $line_aref ( @$cog_file ) {
        $hash{$line_aref->[$func_col]} = $line_aref->[$cog_col];
    }
...
sub create_the_two_files {
    ...
    foreach my $line_aref ( @$gfa_aref ) {
        ...
        if ( $g2c_href->{$line_aref->[0]} && $g2c_href->{$line_aref->[0]} ne "NA" ) {
            ...
            $line_aref->[0] = "No_Clstr";
            $line_aref->[1] = "No_Clstr";
        }
